...
// ============================================================
// WeLoveChile Route Dispatcher v7.4.1 (Fix fallback + varredura vs Google)
//
// MUDANÇA vs v7.0:
...
// CACHE GEOCODING
// ============================================================
var CACHE_KEY = "wlc_geocache_v2"; // v2: invalida geocodes antigos (metrôs errados)
...
function chaveCache(end) {
  // v7.4: normaliza acentos — "Joaquín Montero" e "Joaquin Montero" são o
  // MESMO endereço e precisam unificar na mesma van.
  return end.toLowerCase().trim()
...
async function otimizarVan(pontosVan, saidaTour, invertido, horarioPartida) {
  ...
  // GEOCODING SUSPEITO (v7.4): ponto a >6 km do centróide da van quase sempre
  ...
  ok.forEach(function (p) {
    p.geoSuspeito = distanciaKm(p.lat, p.lng, centV.lat, centV.lng) > 6;
  });
  ...
  // com o MIOLO também ordenado por distância à saída — uma varredura limpa.
  // (v7.4.1: na v7.3 o miolo saía sem ordenação e gerava rotas "nada a ver"
  // sempre que a chamada do Google falhava.)
  var miolo = ok.filter(function (p) { return p !== entrada && p !== saida; })
    .sort(function (a, b) { return distSaidaKm(b, saidaTour) - distSaidaKm(a, saidaTour); });
  if (invertido) miolo.reverse();
  porDist = [entrada].concat(miolo).concat([saida]);
  ...
  if (r.erro) {
    ...
    var arrFb = porDist.concat(falhos);
    arrFb.otimizacaoFalhou = true;
    return arrFb;
  }
  ...
  // GOOGLE × VARREDURA (v7.4.1): em zona densa cheia de mão única, a ordem
  // "tempo-ótima" do Google zigue-zagueia. A varredura (distância à saída,
  // monotônica) é a rota que "faz sentido" pro motorista. Compara as duas
  // com 1 chamada e fica com a de menos km se o tempo não piorar +3 min.
  if (ordenadoLimpo.length >= 6) {
    var varredura = porDist.filter(function (p) { return p.lat; });
    var igual = varredura.length === ordenadoLimpo.length &&
      varredura.every(function (p, ix) { return p === ordenadoLimpo[ix]; });
    if (!igual) {
      var ptsV = varredura.map(function (p) { return { lat: p.lat, lng: p.lng }; });
      var rV = await chamarRoutes(ptsV, horarioPartida, false);
      if (!rV.erro) {
        var tG = (legsAtuais || []).reduce(function (sx, l) { return sx + (l.durationSec || 0); }, 0);
        var kG = (legsAtuais || []).reduce(function (sx, l) { return sx + (l.distanceMeters || 0); }, 0);
        var tV = (rV.legs || []).reduce(function (sx, l) { return sx + (l.durationSec || 0); }, 0);
        var kV = (rV.legs || []).reduce(function (sx, l) { return sx + (l.distanceMeters || 0); }, 0);
        if (kV < kG - 300 && tV - tG < 180) {
          if (typeof console !== "undefined") {
            console.log("🧹 Varredura venceu o Google: " + ((kG - kV) / 1000).toFixed(1) + " km a menos, Δt " + Math.round((tV - tG) / 60) + " min");
          }
          ordenadoLimpo = varredura;
          legsAtuais = rV.legs;
        }
      }
    }
  ...
  // 2-OPT POR KM (v7.3): detecta cruzamentos na ordem vencedora (linha reta,
...
function particionarContiguoViavel(pontosOrd, vansSel, maxParadas) {
  ...
  function buscar(idx, inicio) {
    ...
    if (idx === k) {
      var paxs = [];
      var maxLen = 0;
      var lenGrandes = 0; // paradas em vans >23p: quanto menos, melhor
      for (var i = 0; i < k; i++) {
        var len = cortes[i + 1] - cortes[i];
        if (len > maxParadas) return;
        if (len > maxLen) maxLen = len;
        if (vansSel[i].capacidade > VAN_GRANDE && len > lenGrandes) lenGrandes = len;
      ...
      // Critério v7.4: (1) mínimo de paradas máximas; (2) vans grandes com
      // menos paradas (manobram mal); (3) desbalanceamento de pax.
      if (melhor === null || maxLen < melhor.maxLen ||
        (maxLen === melhor.maxLen && lenGrandes < melhor.lenGrandes) ||
        (maxLen === melhor.maxLen && lenGrandes === melhor.lenGrandes && dif < melhor.dif)) {
        melhor = { cortes: cortes.slice(), paxs: paxs.slice(), dif: dif, maxLen: maxLen, lenGrandes: lenGrandes };
      }
      return;
    }
  ...
  return { tipo: "contiguo", fatias: fatias, dif: melhor.dif, maxLen: melhor.maxLen, lenGrandes: melhor.lenGrandes };
}
...
function clusterizarPorVans(pontosOrd, vansDisponiveis, maxParadas) {
  ...
  if (maxParadas !== Infinity) {
    var comLimite = clusterizarNucleo(pontosOrd, vansDisponiveis, maxParadas, maxParadas);
    if (comLimite && comLimite.tipo !== "erro_encaixe") return comLimite; // pode trazer paradasExcedidas do encaixe
    // Retry sem limite rígido — mas o encaixe MANTÉM o limite como preferência
    var semLimite = clusterizarNucleo(pontosOrd, vansDisponiveis, Infinity, maxParadas);
    if (semLimite && semLimite.fatias) semLimite.paradasExcedidas = true;
    return semLimite;
  }
  return clusterizarNucleo(pontosOrd, vansDisponiveis, Infinity, Infinity);
}

function clusterizarNucleo(pontosOrd, vansDisponiveis, maxParadas, prefParadas) {
  prefParadas = prefParadas || maxParadas;
    ...
    if (todasIguais) {
      ...
      for (var p = 0; p < perms.length; p++) {
        ...
        if (r && (melhor === null ||
          ...
          ((r.maxLen || 0) === (melhor.maxLen || 0) && (r.lenGrandes || 0) < (melhor.lenGrandes || 0)) ||
          ((r.maxLen || 0) === (melhor.maxLen || 0) && (r.lenGrandes || 0) === (melhor.lenGrandes || 0) && r.dif < melhor.dif))) melhor = r;
      }
    }
  ...
  var forcado = particionarEncaixeForcado(pontosOrd, sorted.slice(0, Kmax), prefParadas);
  if (forcado.sobras.length === 0) {
    ...
    return {
      ...
      paradasExcedidas: prefParadas !== Infinity && maxLenE > prefParadas
    };
  }
...
// ============================================================
var VAN_GRANDE = 23; // acima disso o veículo manobra mal: rota tem que ser curta
...
function particionarEncaixeForcado(pontosOrd, vansSel, maxParadas) {
  ...
  var restantes = pontosOrd.slice().sort(function (a, b) { return b.passageiros - a.passageiros; });
  var sobras = [];

  // ============================================================
  // FASE 1 (v7.4): vans GRANDES (>23p) recebem primeiro os MAIORES
  // grupos (≥3p), com coerência geográfica (≤6 km do centróide).
  // O best-fit puro fazia o inverso: grupo de 18p ia pra van de 18
  // (menor folga) e a de 43 virava depósito de 2p — 16 paradas.
  // ============================================================
  fatias.forEach(function (f) {
    if (f.van.capacidade <= VAN_GRANDE) return;
    var achou = true;
    while (achou) {
      achou = false;
      var folga = f.van.capacidade - f.pax;
      if (folga <= 0 || f.pontos.length >= maxParadas) break;
      for (var i = 0; i < restantes.length; i++) {
        var p = restantes[i];
        if (p.passageiros < 3) break; // lista é desc: daqui pra frente só grupos pequenos
        if (p.passageiros > folga) continue;
        if (f.pontos.length > 0) {
          var c = centroideFatia(f.pontos);
          if (distanciaKm(p.lat || c.lat, p.lng || c.lng, c.lat, c.lng) > 6) continue;
        }
        f.pontos.push(p);
        f.pax += p.passageiros;
  ...
  // ============================================================
  // FASE 2: best-fit decreasing no que sobrou, em 3 passadas de
  // preferência: (A) vans pequenas abaixo do limite de paradas →
  // (B) qualquer van abaixo do limite → (C) qualquer van que caiba.
  // Grupos pequenos vão pra vans pequenas; as grandes só completam
  // se não houver alternativa.
  // ============================================================
  function escolherBin(p, respeitarLimite, soPequenas) {
    var melhor = -1, melhorFolga = Infinity, melhorDist = Infinity;
    for (var j = 0; j < fatias.length; j++) {
      if (soPequenas && fatias[j].van.capacidade > VAN_GRANDE) continue;
  ...
  for (var i = 0; i < restantes.length; i++) {
    var p = restantes[i];
    var melhor = escolherBin(p, true, true);
    if (melhor === -1) melhor = escolherBin(p, true, false);
    if (melhor === -1) melhor = escolherBin(p, false, false);
    if (melhor === -1) {
      sobras.push(p);
    } else {
...
// ============================================================
// MESCLA PÓS-GEOCODING (v7.4)
// Textos diferentes ("Hotel X" vs "Rua do Hotel X 123") que geocodificam
// pro MESMO lugar (placeId igual ou <80 m) viram uma reserva só —
// garante que o mesmo endereço nunca se espalhe em vans diferentes.
// ============================================================
function mesclarPorGeocode(enriquecidos) {
  var mapa = {}, ordem = [];
  enriquecidos.forEach(function (r) {
    var k;
    if (r.lat) {
      k = r.placeId ? "pid:" + r.placeId
        : "geo:" + r.lat.toFixed(4) + "," + r.lng.toFixed(4); // ~11 m
    } else {
      k = "txt:" + chaveCache(r.endereco);
    }
    if (mapa[k]) {
      mapa[k].passageiros += r.passageiros;
      mapa[k].origens = (mapa[k].origens || [mapa[k].passageirosOrig || 0]).concat(r.origens || [r.passageiros]);
      if (typeof console !== "undefined") {
        console.log("🔗 Mescla geocode: '" + r.endereco + "' = '" + mapa[k].endereco + "'");
      }
    } else {
      mapa[k] = r;
      if (!mapa[k].origens) mapa[k].origens = [r.passageiros];
      mapa[k].passageirosOrig = r.passageiros;
      ordem.push(k);
    }
  });
  return ordem.map(function (k) { return mapa[k]; });
...
async function processarRotaV7(reservas, saidaTour, invertido, horarioInicio, cache, vansDisponiveis, maxParadas, onProgress) {
  ...
  // Mesmo lugar geocodificado por textos diferentes → uma reserva só
  enriquecidos = mesclarPorGeocode(enriquecidos);
  ...
  for (var fi = 0; fi < clusterResult.fatias.length; fi++) {
    ...
    var otimizacaoFalhou = !!otimizados.otimizacaoFalhou;
    ...
    fatiasComRota.push({
      ...
      otimizacaoFalhou: otimizacaoFalhou
    });
  }
    ...
    try {
      ...
      var rotasFinais = resultado7.fatiasComRota.map(function (fatia) {
        ...
        return {
          ...
          otimizacaoFalhou: !!fatia.otimizacaoFalhou,
  ...
  return (
    <div style={styles.app}>
      ...
      <header style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <div>
            ...
            <div style={styles.subBrand}>Route Dispatcher · Santiago · v7.4.1 · Vans grandes</div>
          </div>
        </div>
                    ...
                    return (
                      <div key={idx} style={dropT ? styles.rotaDrop : styles.rota} onDragOver={onDragOver} onDrop={function () { onDrop(idx); }}>
                        <div style={styles.rotaHead}>
                          ...
                          {rota.otimizacaoFalhou && (
                            <span style={{
                              fontSize: 10, padding: "2px 6px", borderRadius: 3,
                              border: "1px solid rgba(250, 80, 80, 0.4)", color: "#fa5050"
                            }} title="A chamada de otimização do Google falhou nesta van; a ordem mostrada é a varredura por distância à saída. Gere de novo pra tentar otimizar.">
                              ⚠ otimização falhou — ordem aproximada
                            </span>
                          )}
      ...
      <footer style={styles.footer}>
        <span>WeLoveChile · v7.4.1 · Vans grandes</span>
